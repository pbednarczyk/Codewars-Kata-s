<?php

namespace App\Tests\unit\Kata;

use App\Kata\SumsOfParts;
use Codeception\Test\Unit;

/**
 * @author Paweł Bednarczyk <ibednar88@gmail.com>
 */
class SumsOfPartsTest extends Unit
{
    /**
     * @var SumsOfParts
     */
    private $testingObject;

    protected function _before()
    {
        $this->testingObject = new SumsOfParts();
    }

    private function dotest($ls, $expected): void
    {
        $actual = $this->testingObject->partsSums($ls);
        $this->assertEquals($expected, $actual);
    }

    /**
     * @covers SumsOfParts::partsSums
     */
    public function testPartsSumsBasics(): void
    {
        $this->dotest([], [0]);
        $this->dotest([0, 1, 3, 6, 10], [20, 20, 19, 16, 10, 0]);
        $this->dotest([1, 2, 3, 4, 5, 6], [21, 20, 18, 15, 11, 6, 0]);
        $this->dotest([744125, 935, 407, 454, 430, 90, 144, 6710213, 889, 810, 2579358],
                      [10037855, 9293730, 9292795, 9292388, 9291934, 9291504, 9291414, 9291270, 2581057, 2580168, 2579358, 0]);
    }
}